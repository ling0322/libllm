...
/// Sample one label per row of `distribution` `<float>(rows, vocabSize)`, keeping the `top_k`
/// likeliest labels and the smallest set of them whose probability reaches `top_p`.
pub fn sample(distribution: &Tensor, top_k: i32, top_p: f32) -> Result<Tensor> {
    Tensor::produce(|out| unsafe { ffi::fl_sample(distribution.raw, top_k, top_p, out) })
}
